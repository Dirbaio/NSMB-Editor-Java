...
public class FileFilesystemSource extends FilesystemSource
{
    ...
    {
        if(pos < 0) throw new RuntimeException("Seek position out of bounds: "+pos);
        this.pos = pos;
    }
    ...
    {
        if(pos+len > data.length) throw new RuntimeException("Read out of bounds");
        byte[] r = new byte[len];
    ...
    public void setLength(int l)
    {
        byte[] n = new byte[l];
        int c = l;
        if(c > data.length) c = data.length;
